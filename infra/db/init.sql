...
CREATE TABLE IF NOT EXISTS `bom_items` (
  ...
  `qty_per_unit`      DECIMAL(16,6)   DEFAULT NULL COMMENT 'AI成本分析用字段，同 quantity',
...
CREATE TABLE IF NOT EXISTS `supplier_prices` (
  ...
  `purchase_cycle_days`  INT UNSIGNED     DEFAULT NULL COMMENT '采购周期（天）',
  `transport_cycle_days` INT UNSIGNED     DEFAULT NULL COMMENT '运输周期（天）',
