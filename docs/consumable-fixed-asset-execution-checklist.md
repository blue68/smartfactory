...
5. 发布后抽样确认 `/quality/trace` 图片上传与 `/master-data/process-config` 附件上传仍可用，并核对 `uploaded_files` 元数据写入正常。
6. 发布后抽样确认至少 1 份主数据导出和 1 份采购/销售导出，状态字段均为中文，时间字段均为 `YYYY-MM-DD HH:mm:ss`。
